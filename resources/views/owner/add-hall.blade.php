...
@section('body-class', 'page-owner-add-hall')

@section('content')
    <div class="container" style="padding: 110px 20px 40px;">
        <div style="display: flex; align-items: center; justify-content: space-between; gap: 16px; flex-wrap: wrap;">
            <div>
                <h2 style="margin: 0 0 10px;">إضافة قاعة جديدة</h2>
                <p style="margin: 0; color: rgba(255,255,255,0.75);">املأ البيانات التالية لتسجيل قاعة جديدة في حسابك.</p>
            </div>
        ...
        <form method="POST" action="{{ route('owner.halls.store') }}" enctype="multipart/form-data" style="margin-top: 26px; background: rgba(255,255,255,0.06); padding: 30px; border-radius: 18px; border: 1px solid rgba(255,255,255,0.12);">
            @csrf
            <div class="form-row" style="display: flex; gap: 16px; flex-wrap: wrap;">
                <div class="form-group" style="flex: 1; min-width: 280px;">
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" placeholder="مثال: قصر الفرح" required style="width: 100%; padding: 12px 14px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.2); background: rgba(0,0,0,0.4); color: #fff;" />
                </div>
            ...
            <div class="form-row" style="display: flex; gap: 16px; flex-wrap: wrap; margin-top: 16px;">
                <div class="form-group" style="flex: 1; min-width: 280px;">
                    ...
                    <input type="number" name="capacity" value="{{ old('capacity') }}" min="1" placeholder="مثال: 200" required style="width: 100%; padding: 12px 14px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.2); background: rgba(0,0,0,0.4); color: #fff;" />
                </div>

                <div class="form-group" style="flex: 1; min-width: 280px;">
                    <label style="display: block; margin-bottom: 6px; font-weight: 700;">السعر (جنيه)</label>
                    <input type="number" name="price" value="{{ old('price') }}" min="1" placeholder="مثال: 50000" required style="width: 100%; padding: 12px 14px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.2); background: rgba(0,0,0,0.4); color: #fff;" />
                </div>
            </div>
            ...
            <div class="form-row" style="margin-top: 16px; display: flex; gap: 16px; flex-wrap: wrap;">
                <div class="form-group" style="flex: 1; min-width: 280px;">
                    ...
                    <input type="number" name="min_price" value="{{ old('min_price') }}" min="0" placeholder="مثال: 40000" style="width: 100%; padding: 12px 14px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.2); background: rgba(0,0,0,0.4); color: #fff;" />
                </div>

                <div class="form-group" style="flex: 1; min-width: 280px;">
                    <label style="display: block; margin-bottom: 6px; font-weight: 700;">الحد الأقصى للسعر (اختياري)</label>
                    <input type="number" name="max_price" value="{{ old('max_price') }}" min="0" placeholder="مثال: 60000" style="width: 100%; padding: 12px 14px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.2); background: rgba(0,0,0,0.4); color: #fff;" />
                </div>
            </div>
            ...
            <div class="form-row" style="margin-top: 16px; display: flex; gap: 16px; flex-wrap: wrap;">
                <div class="form-group" style="flex: 1; min-width: 280px;">
                    ...
                    <input type="text" name="category" value="{{ old('category') }}" placeholder="مثال: قاعات أفراح، مؤتمرات" style="width: 100%; padding: 12px 14px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.2); background: rgba(0,0,0,0.4); color: #fff;" />
                </div>
            ...
            <div class="form-row" style="margin-top: 16px; display: flex; gap: 16px; flex-wrap: wrap;">
                <div class="form-group" style="flex: 1; min-width: 280px;">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 10px;">
                        ...
                        @foreach($defaultFeatures as $feature)
                            <label style="display: flex; align-items: center; gap: 8px; font-weight: 600;">
                                <input type="checkbox" name="features[]" value="{{ $feature }}" style="width: 22px; height: 22px; min-width: 22px; accent-color: #5d2fb6;" {{ in_array($feature, (array)$oldFeatures) ? 'checked' : '' }} />
                                {{ $feature }}
                            </label>
                        @endforeach
                    </div>
                    ...
                    <input type="text" name="other_features" value="{{ old('other_features') }}" placeholder="اكتب خدمة إضافية (مفصول بفاصلة)" style="width: 100%; padding: 12px 14px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.2); background: rgba(0,0,0,0.4); color: #fff;" />
                </div>

                <div class="form-group" style="flex: 1; min-width: 280px;">
                    <label style="display: block; margin-bottom: 6px; font-weight: 700;">تواريخ غير متاحة (افصل بفواصل)</label>
                    <input type="text" name="unavailable_dates" value="{{ old('unavailable_dates') }}" placeholder="مثال: 2026-03-20, 2026-03-21" style="width: 100%; padding: 12px 14px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.2); background: rgba(0,0,0,0.4); color: #fff;" />
                </div>
            </div>
            ...
            <div style="margin-top: 20px; display: flex; gap: 10px; flex-wrap: wrap;">
                <button type="submit" class="nav-auth-btn btn-primary">حفظ القاعة</button>
